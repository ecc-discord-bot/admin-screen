...
export const Terms = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            fetch("/app/classes", {
                ...
                const classes_data_list = document.getElementById("classes_list") as HTMLSelectElement;

                res.json().then((data) => {
                    setClasses(data["classes"]);

                    classes_data_list.innerHTML = "";

                    //forで回す
                    for (let i = 0; i < data["classes"].length; i++) {
                        //オプションを生成
                        const option = document.createElement("option");
                        option.value = data["classes"][i];
                        option.label = data["classes"][i];

                        //オプションを追加
                        classes_data_list.appendChild(option);
                    }
                });
            });
        } catch (error) {
    ...
    const next = async () => {
        if (!selected && className === "" && Name == "") {
            ...
            let found = classes.includes(className);
        ...
        <section className="font-meiryo flex justify-center bg-gray-50">
            <div className="text-custom-black mx-[10%] my-[5%] w-10/12 bg-white p-[5%] shadow-md shadow-slate-200">
                ...
                <section>
                    ...
                    <p>1年生のみ、1度だけ30日間の体験入部をすることが可能である。</p>
                    <p>
                        体験入部者は第5条の会費を支払う義務は無く、部員と同等にIoT
                        Laboを使用できる。
                    </p>
                    <p>
                ...
                <section>
                    ...
                    <p>同意書の規約は管理者の話し合いの下、追記・改訂する場合がある。</p>
                    <p>その際、部員に再周知し同意書の再提出を求める。</p>
                </section>
                {/* ここまで */}
                ...
                <div className="mt-10">
                    <section className="mt-5 flex justify-center">
                        ...
                        ></input>
                        <datalist id="classes_list"></datalist>
                    </section>
                    <section className="mt-5 flex justify-center">
                        <input
                            type="text"
